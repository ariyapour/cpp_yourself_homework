...
:size_{pixelator::Size{0,0,0}}, image_data_{nullptr}{};
...
pixelator::StbImageDataView::StbImageDataView(const std::string& image_path)
{
    ...
    image_data_ = stbi_load(image_path.c_str(), &size_.col, &size_.row, &size_.channel, 0);
    if (!image_data_) {
...
pixelator::StbImageDataView::StbImageDataView(pixelator::StbImageDataView &&other_image)
...
const ftxui::Color pixelator::StbImageDataView::at(const int& row, const int& col)const{
    const auto index{size_.channel * (row * size_.col + col)};
    ftxui::Color color = ftxui::Color::RGB(image_data_[index], image_data_[index + 1], image_data_[index + 2]);
    return color;
}

pixelator::StbImageDataView & pixelator::StbImageDataView::operator=(StbImageDataView &&object){
